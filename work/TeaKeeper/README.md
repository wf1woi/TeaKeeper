...
构建后的应用位于仓库目录上两级的 `outputs/TeaKeeper.app`。当前构建脚本生成 Apple Silicon 版本，并使用 ad-hoc 签名。
...
The built app is written to `outputs/TeaKeeper.app`, two directory levels above the repository. The current build script targets Apple Silicon and applies an ad-hoc signature.
